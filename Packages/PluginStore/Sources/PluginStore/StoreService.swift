...
public enum StoreService: SuperLog {
    ...
    /// onShutdown 取消，确保不留下后台任务）。
    ///
    /// 线程/actor：在插件持有的可取消 Task 中执行；每个交易先校准本地权益再 finish。
    ...
    /// 按产品 ID 发起购买（PluginStore.purchase 的模块内入口）。
    static func purchase(productID: String) async throws -> Transaction? {
        let products = try await Product.products(for: [productID])
        guard let storekitProduct = products.first else { throw StoreError.canNotGetProducts }
        return try await purchase(storekitProduct)
    }
